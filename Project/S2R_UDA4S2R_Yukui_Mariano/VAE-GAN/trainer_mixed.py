...
class DM_ADA(object):
    ...
    def __init__(self, opt, nclasses, mean, std, mean_1, std_1, source_trainloader, source_valloader, target_trainloader,
                 target_valloader):
        ...
        # Defining optimizers
        self.optimizerD = optim.Adam(self.netD.parameters(), lr=opt.lr_d, betas=(0.5, 0.999))
        self.optimizerG = optim.Adam(self.netG.parameters(), lr=opt.lr_g, betas=(0.5, 0.999))
        self.optimizerF = optim.Adam(self.netF.parameters(), lr=opt.lr_f, betas=(0.5, 0.999))
        self.optimizerC = optim.Adam(self.netC.parameters(), lr=opt.lr, betas=(0.5, 0.999))
        ...
        if opt.checkpoint != None:
            save_path = '%s/model_combine/model_best_model.pth' % (opt.checkpoint)
            checkload = torch.load(save_path)
            self.netF.load_state_dict(checkload['netF'])
            self.netC.load_state_dict(checkload['netC'])
            self.netD.load_state_dict(checkload['netD'])
            self.netG.load_state_dict(checkload['netG'])
            self.optimizerF.load_state_dict(checkload['OptimizerF'])
            self.optimizerC.load_state_dict(checkload['OptimizerC'])
            self.optimizerD.load_state_dict(checkload['OptimizerD'])
            self.optimizerG.load_state_dict(checkload['OptimizerG'])
            #self.curr_epoch = checkload['epoch']
    """
    Validation function
    """
    ...
    def validate(self, epoch):
        ...
        if val_acc > self.best_val:
            ...
            save_path = '%s/model_combine/model_best_model.pth' % (self.opt.outf)
            torch.save({
                'epoch': epoch + 1,
                ...
                "OptimizerF": self.optimizerF.state_dict(),
    ...
    def train(self):
        ...
        for epoch in range(self.opt.nepochs):
            ...
            for i, (datas, datat) in enumerate(itertools.izip(self.source_trainloader, self.target_trainloader)):
                ...
                tgt_inputs_unnorm = (((tgt_inputs * self.std_1[0]) + self.mean_1[0]) - 0.5) * 2
                ...
                # Epoch + Iteration Learning rate scheduling
                if self.opt.lrd:
...
class Sourceonly(object):
    ...
    def validate(self, epoch):
        ...
        if val_acc > self.best_val:
            ...
            save_path = '%s/model_combine/model_best_model_sourceonly.pth' % (self.opt.outf)
            torch.save({
                'epoch': epoch + 1,
                "netF": self.netF.state_dict(),
                "netC": self.netC.state_dict(),
