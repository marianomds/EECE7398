...
def main(opt):
    ...
    try:
        ...
        pass
    try:
        os.makedirs(os.path.join(opt.outf, 'model_combine'))
    except OSError:
        pass
    try:
...
def run(opt):
    ...
    mean_1 = np.array([0.4914, 0.4822, 0.4465])
    std_1 = np.array([0.2023, 0.1994, 0.2010])
    ...
    transform_source = transforms.Compose([transforms.Resize(resize_shape), transforms.ToTensor(), transforms.Normalize(mean, std)])
    transform_target = transforms.Compose([transforms.Resize(resize_shape), transforms.ToTensor(), transforms.Normalize(mean_1, std_1)])
    ...
    if opt.method == 'DM-ADA':
        DM_ADA_trainer = trainer_mixed.DM_ADA(opt, nclasses, mean, std, mean_1, std_1, source_trainloader,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataroot', type=str, default='../dataset/', help='path to source dataset')
    ...
    parser.add_argument('--lr_f', type=float, default=0.0005, help='learning rate, default=0.0005')
    parser.add_argument('--gpu', type=str, default='0,1', help='GPU to use, -1 for CPU training')
    parser.add_argument('--outf', default='./results', help='folder to output images and model checkpoints')
    parser.add_argument('--checkpoint', default=None, help='folder to load model checkpoints')
    parser.add_argument('--method', default='DM-ADA', help='Method to train| DM-ADA, sourceonly')
    parser.add_argument('--manualSeed', type=int, default = 703, help='manual seed')
    ...
    parser.add_argument('--lrd', type=float, default=0.0001, help='learning rate decay, default=0.0001') # follow this dataset, 0.000001
    ...
    parser.add_argument('--source_dataset', default='sgan_svhn', help='name of the source dataset')
    parser.add_argument('--target_dataset', default='svhn', help='name of the target dataset')
