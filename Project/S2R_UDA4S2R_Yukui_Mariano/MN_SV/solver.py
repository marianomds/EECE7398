...
class Solver(object):
    def __init__(self, config, svhn_loader, mnist_loader, svhn_train_loader, mnist_train_loader, svhn_test_loader, mnist_test_loader):
        self.svhn_loader = svhn_loader
        self.mnist_loader = mnist_loader
        self.svhn_train_loader = svhn_train_loader
        self.mnist_train_loader = mnist_train_loader
        self.svhn_test_loader = svhn_test_loader
        self.mnist_test_loader = mnist_test_loader
        self.g12 = None
        self.g21 = None
        self.d1 = None
        self.d2 = None
    ...
    def load_model(self):
        ...
        self.g12.load_state_dict(torch.load(os.path.join(self.best_model_path, 'g12-%d.pkl' %(73500))))
        self.g21.load_state_dict(torch.load(os.path.join(self.best_model_path, 'g21-%d.pkl' % (73500))))
        self.d1.load_state_dict(torch.load(os.path.join(self.best_model_path, 'd1-%d.pkl' % (73500))))
        self.d2.load_state_dict(torch.load(os.path.join(self.best_model_path, 'd2-%d.pkl' % (73500))))
    ...
    def sample(self):
        if self.sample_mode == 'MNSV':
            mnist_iter = iter(self.mnist_train_loader)
            self.load_model()
            count = 0
            for data, label in mnist_iter:
                fixed_mnist = self.to_var(data)
                label = label.numpy()
                fake_svhn = self.g12(fixed_mnist)
                fake_svhn = self.to_data(fake_svhn)
                #print(fake_svhn[299][2][31][31])
                image_one = self.one_images(fake_svhn)
                path = os.path.join(self.output_path, 'trainset', '%d' % (label[0]), '%d.png' % (count))
                scipy.misc.imsave(path, image_one)
                count = count + 1
                print ('saved trainset %s' % path)

            mnist_test_iter = iter(self.mnist_test_loader)
            count = 0
            for data,label in mnist_test_iter:
                fixed_mnist = self.to_var(data)
                label = label.numpy()
                fake_svhn = self.g12(fixed_mnist)
                fake_svhn = self.to_data(fake_svhn)
                image_one = self.one_images(fake_svhn)
                path = os.path.join(self.output_path, 'testset', '%d' % (label[0]), '%d.png' % (count))
                scipy.misc.imsave(path, image_one)
                count = count + 1
                print ('saved testset %s' % path)

        elif self.sample_mode == 'SVMV':
            svhn_iter = iter(self.svhn_train_loader)
            self.load_model()
            count = 0
            for data,label in svhn_iter:
                fixed_svhn = self.to_var(data)
                label = label.numpy()
                fake_mnist = self.g21(fixed_svhn)
                fake_mnist = self.to_data(fake_mnist)
                image_one = self.one_images(fake_mnist)
                path = os.path.join(self.output_path, 'trainset', '%d' % (label[0]), '%d.png' % (count))
                scipy.misc.imsave(path, image_one)
                count = count + 1
                print ('saved trainset %s' % path)
            svhn_test_iter = iter(self.svhn_test_loader)
            count = 0
            for data,label in svhn_test_iter:
                fixed_mnist = self.to_var(data)
                label = label.numpy()
                fake_svhn = self.g12(fixed_mnist)
                fake_svhn = self.to_data(fake_svhn)
